--[[
    Nighttime V3.5 - Full Roblox Native GUI
    All-in-one: Settings panel + logic runs inside Roblox
    Toggle GUI: RightShift
    Panic (destroy): Delete
]]
...
Corner(12, TitleBar)
-- fix bottom corners
New("Frame", {Size=UDim2.new(1,0,0,12),Position=UDim2.new(0,0,1,-12),BackgroundColor3=PANEL,BorderSizePixel=0}, TitleBar)
...
Corner(7, IconBox)
New("TextLabel", {Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="⚡",TextColor3=Color3.new(1,1,1),Font=Enum.Font.GothamBold,TextSize=15}, IconBox)

New("TextLabel", {
    Size=UDim2.new(0,200,0,18),Position=UDim2.new(0,50,0,8),
    BackgroundTransparency=1,Text="NIGHTTIME",
    TextColor3=Color3.new(1,1,1),Font=Enum.Font.GothamBold,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left
}, TitleBar)
New("TextLabel", {
    Size=UDim2.new(0,200,0,13),Position=UDim2.new(0,50,0,24),
    BackgroundTransparency=1,Text="V3.5 PRIVATE",
    TextColor3=ACCENT,Font=Enum.Font.GothamBold,TextSize=9,TextXAlignment=Enum.TextXAlignment.Left
}, TitleBar)
...
-- right border line
New("Frame", {Size=UDim2.new(0,1,1,0),Position=UDim2.new(1,-1,0,0),BackgroundColor3=BORDER,BorderSizePixel=0}, Sidebar)
...
New("UIPadding", {PaddingTop=UDim.new(0,10),PaddingLeft=UDim.new(0,6),PaddingRight=UDim.new(0,6)}, SideScroll)
...
for _, def in ipairs(tabDefs) do
    ...
    New("UIPadding", {PaddingTop=UDim.new(0,12),PaddingLeft=UDim.new(0,12),PaddingRight=UDim.new(0,14),PaddingBottom=UDim.new(0,12)}, page)
...
local function Section(parent, title, order)
    ...
    New("UIPadding", {PaddingTop=UDim.new(0,12),PaddingBottom=UDim.new(0,12),PaddingLeft=UDim.new(0,14),PaddingRight=UDim.new(0,14)}, wrap)
    ...
    New("Frame", {
        Size=UDim2.new(1,0,0,1),BackgroundColor3=BORDER,BorderSizePixel=0,LayoutOrder=1
    }, inner)
...
local function Toggle(parent, label, key, order)
    ...
    local row = New("Frame", {
        Size=UDim2.new(1,0,0,26),BackgroundTransparency=1,LayoutOrder=(order or 1)+1
    }, parent)
    ...
    local track = New("Frame", {
        ...
        BackgroundColor3=val and ACCENT or Color3.fromRGB(40,40,48),BorderSizePixel=0
    }, row)
    Corner(10, track)
...
local function Slider(parent, label, key, min, max, step, order)
    step = step or 1
    local row = New("Frame", {
        Size=UDim2.new(1,0,0,46),BackgroundTransparency=1,LayoutOrder=(order or 1)+1
    }, parent)

    New("TextLabel", {
        Size=UDim2.new(1,-50,0,16),BackgroundTransparency=1,
        Text=label,TextColor3=TEXT,Font=Enum.Font.Gotham,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left
    }, row)

    local valLbl = New("TextLabel", {
        Size=UDim2.new(0,46,0,16),Position=UDim2.new(1,-46,0,0),
        BackgroundTransparency=1,Text=tostring(Settings[key]),
        TextColor3=ACCENT,Font=Enum.Font.GothamBold,TextSize=11,TextXAlignment=Enum.TextXAlignment.Right
    }, row)
...
local function Dropdown(parent, label, key, options, order)
    ...
    New("TextLabel", {
        ...
        Text=label,TextColor3=TEXT,Font=Enum.Font.Gotham,TextSize=12,
...
do
    ...
    Dropdown(s1,"Hit Part","HitPart",{"Head","HumanoidRootPart","UpperTorso","LowerTorso"},7)
    ...
    Slider(s2,"Prediction Amount","PredictionAmt",0,0.5,0.01,4)
...
RunService.Heartbeat:Connect(function()
    if Settings.Fullbright then Lighting.Brightness=2; Lighting.FogEnd=9e8 end
    if Settings.NoFog then Lighting.FogEnd=9e8; Lighting.FogStart=9e8 end
    if Settings.CustomTime then Lighting.ClockTime=Settings.TimeValue end
end)
...
local function getESP(plr)
    if not espPool[plr] then
        ...
        espPool[plr].box.Filled = false
        espPool[plr].box.Thickness = 1
        espPool[plr].name.Size = 13
        espPool[plr].name.Center = true
        espPool[plr].name.Outline = true
        espPool[plr].hpbg.Filled = true
        espPool[plr].hpfg.Filled = true
        espPool[plr].hpbg.Color = Color3.fromRGB(20,20,20)
        espPool[plr].hpfg.Color = Color3.fromRGB(50,220,80)
        espPool[plr].tracer.Thickness = 1
        for _,d in pairs(espPool[plr]) do d.Visible=false end
    end
    return espPool[plr]
end
...
RunService.RenderStepped:Connect(function()
    ...
    local dirs = {
        {Vector2.new(cx-sz-g,cy), Vector2.new(cx-g,cy)},
        {Vector2.new(cx+g,cy),    Vector2.new(cx+sz+g,cy)},
        {Vector2.new(cx,cy-sz-g), Vector2.new(cx,cy-g)},
        {Vector2.new(cx,cy+g),    Vector2.new(cx,cy+sz+g)},
    }
    for i,d in ipairs(dirs) do
        chLines[i].From=d[1]; chLines[i].To=d[2]
        chLines[i].Color=col; chLines[i].Visible=Settings.CrosshairOn
    end
    chDot.Position=Vector2.new(cx,cy); chDot.Color=col
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if not onscreen then for _,d in pairs(objs) do d.Visible=false end continue end
        ...
        objs.hpfg.Color    = Color3.fromRGB(math.floor((1-hpPct)*255), math.floor(hpPct*200+55), 50)
    end
end)
...
local function getBestTarget()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        local char = plr.Character
        local part = char and (char:FindFirstChild(Settings.HitPart) or char:FindFirstChild("HumanoidRootPart"))
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        ...
        if Settings.WallCheck then
            ...
            local ray = workspace:Raycast(Camera.CFrame.Position, dir, RaycastParams.new())
            if ray and not char:IsAncestorOf(ray.Instance) then continue end
        end
        local d = (Vector2.new(sp.X,sp.Y)-center).Magnitude
        if d<Settings.FOV and d<bestDist then best=part; bestDist=d end
    end
    return best
end
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.Smoothing then
        ...
        local lerpFactor = math.clamp(1 / Settings.SmoothFactor, 0.01, 1)
        local smoothed = currentLook:Lerp(desiredLook, lerpFactor)
